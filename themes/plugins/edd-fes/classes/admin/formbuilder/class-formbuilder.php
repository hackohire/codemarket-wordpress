...
function fes_formbuilder_save() {
	if ( get_the_ID() == EDD_FES()->helper->get_option( 'fes-submission-form', false ) ) {
		...
		<script>
		(function($) {
			var menu = $( "#toplevel_page_fes-vendors" );

			menu.removeClass( 'wp-not-current-submenu' );
			menu.addClass( 'wp-has-current-submenu' );

			var link = $( "#toplevel_page_fes-vendors" ).find( ".toplevel_page_fes-vendors");
	  ...
	  <?php
		...
		foreach ( $order as $type => $index ) {
			if ( count( $index )  >= 1 ) {
				...
				$url_args  = array(
					'utm_source'   => 'settings',
					'utm_medium'   => 'fes_form_custom_fields',
					'utm_campaign' => 'admin',
				);
				$url   = esc_url( add_query_arg( $url_args, 'https://easydigitaldownloads.com/downloads/tag/fes-custom-form-fields/' ) );
				$message = sprintf( __( 'There are no extension fields for this form. Add some with <a href="%s">these extensions</a>.', 'edd_fes' ), $url );
