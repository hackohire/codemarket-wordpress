...
class FES_Login_Form extends FES_Form {
	...
	public function extending_constructor() {
		...
		add_action( 'fes_render_' . $this->name() . '_after_fields', array( $this, 'lost_password_link' ) );
	}
	...
	/**
	 * Add a `Lost Password?` link inside the login form.
	 *
	 * @access public
	 * @since 2.6
	 */
	public function lost_password_link( $output ) {
		$lost_password_link = '<a href="' . wp_lostpassword_url() . '" id="fes_lost_password_link" title="' . __( 'Lost Password?', 'edd_fes' ) . '">' . __( 'Lost Password?', 'edd_fes' ) . '</a>';

		/**
		 * Allow the `Lost Password?` link to be filtered.
		 *
		 * @since 2.6
		 *
		 * @param string $lost_password_link Lost password link HTML.
		 */
		return apply_filters( 'edd_fes_lost_password_link', $lost_password_link );
	}
}