...
function user_list_callback($atts)
{
    ...
    <div class="container">
        <div class="table-responsive">	<table class="table fes-table table-condensed table-striped tablesorter {sortlist: [[2,0]]}" id="myTable user_list">
		    ...
		    {
			$vender_post_count = count_user_posts($user->ID, 'download');

			//$user_profile_link = get_site_url() . '/profile/?request=' . $user->ID;
			$user_profile_link = get_site_url() . '/user-profile/?user_id=' . $user->ID;
			
			if ($vender_post_count > 0)
			{
			    ?>
	    		<tr>
	    		    <td><a href="<?php echo $user_profile_link; ?>"><?php echo esc_html($user->display_name) ?></a></td>
	    		    <td class="fes-order-list-td widget"><?php echo $vender_post_count; ?></td>
	    		</tr>
			    <?php
			}
		    }
