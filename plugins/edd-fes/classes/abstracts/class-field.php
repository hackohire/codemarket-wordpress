...
class FES_Field {
	 ...
	 *
	 * @access public
	 * @since  2.6 - Dispatch to FES_Field::is_valid()
	 *
	 * @see    FES_Field::is_valid()
	 *
	...
	public function validate( $values = array(), $save_id = -2, $user_id = -2 ) {
		...
		$validation = $this->is_valid( $values, $save_id, $user_id );

		if ( is_wp_error( $validation ) ) {
			$return_value = $validation->get_error_message( 'edd_fes_validation_failed' );
		}

		return apply_filters( 'fes_validate_' . $this->template() . '_field', $return_value, $values, $this->name(), $save_id, $user_id );
	}
	 ...
	 * @param int   $user_id User ID.
	 *
	 * @return bool|WP_Error True if validation passed, WP_Error otherwise.
	 */
	public function is_valid( $values = array(), $save_id = -2, $user_id = -2 ) {
		$name = $this->name();

		if ( empty( $values[ $name ] ) && $this->required() ) {
			return new WP_Error( 'edd_fes_validation_failed', __( 'Please fill out this field.', 'edd_fes' ) );
		}

		/**
		 * Filter the validation value. Default is true.
		 *
		 * @since 2.6
		 *
		 * @param array  $values  Field values.
		 ...
		 * @param int    $user_id User ID.
		 */
		return apply_filters( 'edd_fes_' . $this->template() . 'is_valid', true, $values, $name, $save_id, $user_id );
	}
