...
class FES_Form {
	...
	public function __construct( $key = 0, $by = 'id', $save_id = null ) {
		if ( 0 === $key ) { // let's fallback to login form if something catastrophic happens
	 ...
	 * If adding a support do something like $supports = $field->supports; $supports['something'] = true;
	 * $field->add_support( $support ); If removing do $supports = $field->supports; unset($supports['something']);
	 * $field->add_support( $support );
	 */
	...
	public function render_form_frontend( $user_id = -2, $readonly = -2 ) {
		...
		if ( ! empty( $fields ) && $count > 0 ) {
			...
			/**
			 * Allow content before the fields are rendered.
			 *
			 * @since 2.6
			 */
			$output .= apply_filters( 'fes_render_' . $this->name() . '_before_fields', '', $this, $user_id, $readonly );
			...
			$output .= apply_filters( 'fes_render_' . $this->name() . '_form_frontend_fields_before_submit_button', '', $field, $this, $user_id, $readonly );
			...
			$output .= apply_filters( 'fes_render_' . $this->name() . '_form_frontend_fields_after_submit_button', '', $field, $this, $user_id, $readonly );

			/**
			 * Allow content after the fields are rendered.
			 *
			 * @since 2.6
			 */
			$output .= apply_filters( 'fes_render_' . $this->name() . '_after_fields', '', $this, $user_id, $readonly );
	...
	public function legend() {
		return apply_filters( 'fes_' . $this->name() . '_form_legend', '<legend class="fes-form-legend" id="fes-' . $this->name() . '-form-title">' . $this->title() . '</legend>', $this );
	}
