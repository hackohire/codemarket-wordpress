...
class FES_Download_Table {
	...
	public function __construct() {
		...
		add_action( 'handle_bulk_actions-edit-download', array( $this, 'do_bulk_actions' ), 10, 3 );

		add_filter( 'bulk_actions-edit-download', array( $this, 'register_bulk_action' ) );
		add_filter( 'post_row_actions', array( $this, 'remove_quick_edit' ), 10, 2 );
		add_filter( 'post_row_actions', array( $this, 'add_approve_row_action' ), 10, 2 );
	...
	public function add_approve_row_action( $actions, $post ) {
		if ( 'download' === $post->post_type ) {
			if ( 'pending' === $post->post_status && current_user_can( 'publish_posts' ) ) {
				$url = esc_url( wp_nonce_url( add_query_arg( 'approve_download', $post->ID ), 'approve_download' ) );

				$actions['fes-approve'] = sprintf( '<a href="%1$s">%2$s</a>', $url, esc_html( __( 'Approve', 'edd_fes' ) ) );
			}
		}
	...
	/**
	 * Register bulk action for approving multiple downloads.
	 *
	 * @access public
	 * @since  2.6
	 */
	public function register_bulk_action( $bulk_actions ) {
		$bulk_actions['approve_downloads'] = sprintf( __( 'Approve %s', 'edd_fes' ), edd_get_label_plural() );
	 ...
	 *
	 * @access public
	 * @since  2.6 - Implementation rewritten to follow correct way of handling bulk actions.
	 *
	 * @param string $redirect_url The redirect URL.
	 * @param string $redirect_url The action being taken.
	 * @param array  $post_ids     The post IDs to take the action on.
	 *
	 * @return string $redirect_url The redirect URL.
	 */
	public function do_bulk_actions( $redirect_url, $doaction, $post_ids ) {
		if ( 'approve_downloads' !== $doaction ) {
			return $redirect_url;
		}
		...
		$redirect_url = add_query_arg( 'approved_downloads', $approved_downloads, $redirect_url );
