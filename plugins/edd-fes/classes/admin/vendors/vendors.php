...
function fes_vendors_view( $vendor ) { ?>
		...
		<form id="edit-vendor-info" method="post" action="<?php echo admin_url( 'admin.php?page=fes-vendors&view=overview&id=' . $vendor->id ); ?>">
			...
			<div class="vendor-info">
				...
				<div class="vendor-main-wrapper left">
					...
					<span class="vendor-automatic-approval info-item edit-item"><input data-key="automatic_approval" id="automatic_approval" name="vendorinfo[automatic_approval]" type="checkbox" <?php checked( $vendor->automatic_approval, true ); ?> /> <label for="automatic_approval"><?php printf( 'Automatically approve all %s', EDD_FES()->helper->get_product_constant_name( true, false ) ); ?></label></span>
					<span class="vendor-automatic-approval info-item editable" data-key="automatic_approval"><?php $vendor->automatic_approval ? _e( 'Automatic approval enabled', 'edd_fes' ) : _e( 'Automatic approval disabled', 'edd_fes' ); ?></span>
				</div>
	...
	<div id="vendor-tables-wrapper" class="vendor-section">
		...
		<?php if ( EDD_FES()->integrations->is_commissions_active() ) { ?>
			...
			<?php $unpaid_commissions = eddc_get_unpaid_commissions( array(
				'user_id' => $vendor->user_id,
				'number'  => 5,
			) ); ?>
						...
						<?php foreach ( $unpaid_commissions as $commission ) : ?>
							<tr>
								...
								if ( class_exists( 'EDD_Commission' ) ) {
									$item_name = get_the_title( $commission->download_id );
		...
		<table class="wp-list-table widefat striped downloads">
			...
			<tbody>
				<?php if ( ! empty( $products ) ) : ?>
					<?php $counter = 0; ?>
					<?php foreach ( $products as $product ) : if ( ! get_post( $product ) ) {
						continue;
					} ?>
						<?php
						$counter++;
...
function fes_vendor_products_view( $vendor ) {
	...
	<div id="vendor-products-wrapper">
		...
		<p><?php echo $product_count . ' ' . $product_label; ?></p>
		...
		if ( ! empty( $products ) ) { ?>
			<table class="widefat" id="fes-all-products">
				...
				<tbody>
					...
					foreach ( $products as $product ) :
						?>
						<tr <?php if ( $i == 'alternate' ) {
							echo 'class="alternate"';
						} ?>>
							<td><?php echo esc_html( $product['ID'] ); ?></td>
							<td>
								<a href="<?php echo esc_html( esc_url( admin_url( 'post.php?post=' . $product['ID'] . '&action=edit' ) ) ); ?>"><?php echo esc_html( $product['title'] ); ?></a>
							</td>
			...
			<?php
			EDD_FES()->dashboard->product_list_pagination( $vendor->user_id );
		} else {
		...
		<div id="post-body-content">
			...
			<div class="postbox edd-export-pdf-sales-earnings">
				<div class="inside">
					...
					<p>
						<a class="button" href="<?php echo wp_nonce_url( add_query_arg( array( 'edd-action' => 'generate_fes_pdf' ) ), 'edd_generate_fes_pdf' ); ?>"><?php _e( 'Generate PDF', 'edd_fes' ); ?></a>
					</p>
				</div>
			</div>
...
/**
 * Renders the vendor delete view.
 *
 * @access public
 * @since  2.6
 *
 ...
 * @return void
 */
function fes_vendor_delete_view( $vendor ) {
	$users = get_users( array( 'fields' => array( 'ID', 'user_login', 'display_name' ) ) );
	$options = array();
	$options[0] = __( 'Select a user', 'edd_fes' );

	foreach ( $users as $user ) {
		$display = sprintf( _x( '%1$s (%2$s)', 'user dropdown' ), $user->display_name, $user->user_login );
		$options[ $user->ID ] = $display;
	}
	...
	$args = array(
		...
		'name'             => 'fes-user-list',
		'id'               => 'fes_user_list',
		'placeholder'      => __( 'Type to search all users', 'edd_fes' ),
		'multiple'         => false,
	...
	?>
	<div id="vendor-delete-wrapper">
		<form id="delete-vendor" method="post" action="<?php echo admin_url( 'admin.php?page=fes-vendors&view=delete&id=' . $vendor->id ); ?>">

			<div class="vendor-notes-header">
				<?php echo get_avatar( $vendor->email, 30 ); ?> <span><?php echo $vendor->name; ?></span>
			</div>

			<div class="vendor-info delete-vendor">

				<span class="delete-vendor-options">
					<p>
						<?php echo EDD()->html->checkbox( array( 'name' => 'fes-vendor-delete-confirm' ) ); ?>
						<label for="fes-vendor-delete-confirm"><?php _e( 'Are you sure you want to delete this vendor?', 'edd_fes' ); ?></label>
					</p>

					<p>
						<?php _e( 'What should be done with the products owned by this vendor?', 'edd_fes' ); ?>
					</p>

					<p>
						<input type="radio" id="fes-delete-all-content" name="fes-delete-option" value="delete" disabled="disabled" />
						<label for="fes-delete-all-content"><?php _e( 'Delete all products', 'edd_fes' ); ?></label>
					</p>

					<p>
						<input type="radio" id="fes-attribute-to-other" name="fes-delete-option" value="other" disabled="disabled" />
						<label for="fes-attribute-to-other"><?php _e( 'Attribute all products to:', 'edd_fes' ); ?> <?php echo EDD()->html->select( $args ); ?></label>
					</p>

					<?php do_action( 'fes_vendor_delete_inputs', $vendor ); ?>
				</span>

				<span id="vendor-edit-actions">
					<input type="hidden" name="vendor_id" value="<?php echo $vendor->id; ?>" />
					<?php wp_nonce_field( 'fes_delete_vendor', '_wpnonce', false, true ); ?>
					<input type="hidden" name="edd_action" value="fes_delete_vendor" />
					<input type="submit" disabled="disabled" id="fes-delete-vendor" class="button-primary" value="<?php _e( 'Delete Vendor', 'edd_fes' ); ?>" />
					<a id="fes-delete-vendor-cancel" href="<?php echo admin_url( 'admin.php?page=fes-vendors&view=overview&id=' . $vendor->id ); ?>" class="delete"><?php _e( 'Cancel', 'edd_fes' ); ?></a>
				</span>
