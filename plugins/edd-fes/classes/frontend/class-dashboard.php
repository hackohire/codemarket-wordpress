...
class FES_Dashboard
{
     ...
     *
     * Displays the vendor dashboard page. This is the output of the `fes_vendor_dashboard shortcode`.
     *
     * @since  2.0.0
     * @access public
     *
        ...
        else
        {
            ...
            switch($task)
            {
                ...
                case 'new-product':
                    ...
                    break;
                case 'help-request':
                    EDD_FES()->templates->fes_get_template_part('frontend', 'help-request');
                    break;
                case 'edit-help-request':
                    EDD_FES()->templates->fes_get_template_part('frontend', 'help-request');
                    break;
                case 'view-help-request':
                    EDD_FES()->templates->fes_get_template_part('frontend', 'view-help-request');
                    break;
                case 'edit-product':
     ...
     *
     * This function gets the price of a product for use on the vendor dashboard page.
     *
     * @since  2.0.0
     * @access public
     *
    ...
    public function product_list_actions($product_id)
    {
        ...
        <a href="<?php echo add_query_arg(array(
            'task' => 'edit-product',
            'post_id' => $product_id,
        ), get_permalink()); ?>" title="<?php _e('Edit', 'edd_fes'); ?>" class="edd-fes-action edit-product-fes"><?php _e('Edit', 'edd_fes'); ?></a>
    <?php endif; ?>
        ...
        <a href="<?php echo add_query_arg(array(
            'task' => 'delete-product',
            'post_id' => $product_id,
        ), get_permalink()); ?>" title="<?php _e('Delete', 'edd_fes'); ?>" class="edd-fes-action edit-product-fes"><?php _e('Delete', 'edd_fes'); ?></a>
    <?php endif;
    }
    ...
    public function product_list_date($product_id)
    {
         ...
         * Get Product Date.
         *
         * The product date retrieved by FES for a particular product post ID.
         *
         * @since 2.0.0
         *
    ...
    public function order_list_actions($order_id)
    {
        ...
        <a href="<?php echo add_query_arg(array(
            'task' => 'edit-order',
            'order_id' => $order_id,
        ), get_permalink()); ?>" title="<?php _e('View', 'edd_fes'); ?>" class="view-order-fes"><?php _e('View', 'edd_fes'); ?></a>
        <?php
    }
