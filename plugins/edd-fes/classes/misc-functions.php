...
function fes_after_edd_system_info( $return ) {
	...
	// get all settings that have IDs prefixed with 'fes-'
	foreach( $fes_settings as $key => $value ) {
		$fes_settings_keys = explode( '-', $key );
		if ( $fes_settings_keys[0] == 'fes' ) {
			$fes_all_settings[] = $key . ': ' . $value;
		}
	}
	ob_start();
	?>
...
FES Settings:
<?php
foreach ( $fes_all_settings as $fessetting ) {
	echo '                          ' . $fessetting . "\n";
}
