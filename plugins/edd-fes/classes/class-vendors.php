 ...
 * @since      2.3.0
 *
 * @todo       Simply and deprecate some of these functions that are no longer used.
 */
...
class FES_Vendors {
	...
	public function prevent_admin_access() {
		...
		if ( // Look for the presence of /wp-admin/ in the url
			stripos( $_SERVER['REQUEST_URI'], '/wp-admin/' ) !== false && // Allow calls to async-upload.php
			stripos( $_SERVER['REQUEST_URI'], 'async-upload.php' ) == false && // Allow calls to media-upload.php
			stripos( $_SERVER['REQUEST_URI'], 'media-upload.php' ) == false && // Allow calls to admin-ajax.php
	 ...
	 * @access public
	 *
	 * @param string|bool $email_or_username The username or email.
	 *
	 * @return int The user id.
	 */
	public function get_user_id( $email_or_username = false ) {
		if ( false === $email_or_username ) {
			return false;
		}
	...
	public function vendor_can_view_receipt( $user_can_view, $edd_receipt_id ) {
		...
		if ( $payment_id ) {
			...
			foreach ( $cart as $item ) {
				...
				if ( get_current_user_id() == $item->post_author ) {
					$user_can_view = true;
	 ...
	 *
	 * @return array|bool The pending products, false otherwise.
	 */
	...
	public function get_all_products( $user_id = false, $status = array( 'draft', 'pending', 'publish', 'trash', 'future', 'private' ) ) {
		...
		$data = $this->array_msort( $data, array(
			'status' => SORT_ASC,
			'title'  => SORT_ASC,
			'sales'  => SORT_DESC,
			'ID'     => SORT_ASC,
	 ...
	 *
	 * @param int|bool $user_id The user id.
	 * @param array    $status  The post status to get products of.
	 *
	 ...
	 *
	 * @param int   $user_id The user id.
	 * @param array $args    The order status to get orders of.
	 *
	...
	/**
	 * Retrieve a list of vendors and their profiles.
	 *
	 * @access public
	 * @since  2.6
	 *
	 ...
	 */
	public function get_vendor_profiles( $args = array() ) {
		$defaults = array(
			'status' => 'approved',
			'number' => 10,
			'offset' => 0,
		...
		foreach ( $vendors as $vendor ) {
			$vendor->avatar        = get_user_meta( $vendor->user_id, 'user_avatar', true );
			$vendor->store_url     = $this->get_vendor_store_url( $vendor->user_id );
			$vendor->store_name    = get_user_meta( $vendor->user_id, 'name_of_store', true );
			$vendor->description   = get_user_meta( $vendor->user_id, 'description', true );
			$vendor->contact_email = get_user_meta( $vendor->user_id, 'email_to_use_for_contact_form', true );
		}

		/**
		 * Filter the vendor profiles.
		 *
		 * @param array $vendors Vendor profiles.
		 */
		return apply_filters( 'edd_fes_get_vendor_profiles', $vendors );
	}
