...
/**
 * FES_Telephone_Number_Field Class.
 *
 * @since 2.6
 *
 * @see   FES_Text_Field
 */
class FES_Telephone_Number_Field extends FES_Text_Field {
	...
	public function set_title() {
		$this->supports['title'] = apply_filters( 'fes_' . $this->name() . '_field_title', _x( 'Telephone Number', 'FES Field title translation', 'edd_fes' ) );
	}
	...
	public function validate( $values = array(), $save_id = -2, $user_id = -2 ) {
		...
		if ( $this->required() ) {
			if ( ! empty( $values[ $name ] ) ) {
				$matches = preg_match( '/^[\d -]+$/', $values[ $name ] );
				if ( empty( $matches ) ) {
					$return_value = __( 'Please enter a valid telephone number.', 'edd_fes' );
				}
			}
		}
