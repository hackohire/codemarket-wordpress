...
class WCFM_Fields {
  ...
  function wpeditor_input( $field ) {
    ...
    $field['teeny']     = isset( $field['teeny'] ) ? $field['teeny'] : false;
    ...
    $field['teeny']  = apply_filters( 'wcfm_is_allow_wpeditor_teeny_mode', $field['teeny'] );
    ...
    wp_editor(stripslashes($field['value']), $field['id'], $settings = array('textarea_name' => $field['name'], 'textarea_rows' => $field['rows'], 'editor_class' => $field['class'], 'tinymce' => $field['tinymce'], 'teeny' => $field['teeny'], 'media_buttons' => $field['media_buttons'], 'quicktags' => $field['quicktags'], 'text_direction' => 'RTL' ) );
  ...
  public function multi_input($field) {
    ...
    if(!empty($field['options'])) {
      for($blockCount = 0; $blockCount < $eleCount; $blockCount++) {
        ...
        printf('<span class="multi_input_block_manupulate remove_multi_input_block wcfmfa fa-times-circle"></span>
                <span class="add_multi_input_block multi_input_block_manupulate wcfmfa fa-plus-circle"></span></div>');
      }
    }
